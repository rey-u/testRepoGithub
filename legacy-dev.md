...
Part of planning involves determining who the best point of contact is for content development, support, or validation.
...
|Subtask|Begins with|Ends by
| :-- | :-- | :-- 
|Align scenario to pain points|- Voice of the customer is tracked<br/>- When Customers pain points are identified and prioritized.<br/>- To help decide the structure of content. What does the customer need first?|Identified scenarios are aligned with pain points.
|Determine content structure|- Before writing documentation for steps that are common across other services or across other articles within same doc set.<br/>- For developer content, writing steps for same feature but on multiple IDE, Shell. What part to pivot in TOC to handle variations that come up with what customers need and how they want to experience.|- Content types are identified<br/>- Structure of Table of Content(TOC) is defined<br/>- Shared or common content is identified - to use include files<br/>- Content for zone pivots is identified.
|Cross service content|- When there is any cross service content involved<br/>- If you need to place an article on two different TOCs/module in two different learning paths/collections|- Content dependencies across services/products are identified<br/>- Plan is developed to figure out how content might work across services  
|Write customer intent statements|Content structure(TOC) and content types are identified|Include the tasks and a customer intent statement for each article in TOC.
|Identify SEO terms||
|Identify terms to use throughout *Docs*|Plan approved for documentation for new product or service|- Finalized terminology that will be used consistently across documentation<br/>- Finalized Brand name to use on public facing sites<br/>- Approved Metadata values added to allow list 


## Align scenarios to pain points
- https://review.docs.microsoft.com/help/contribute/contribute-get-started-mvc (top tasks; not very clear existing guidance)

Documentation as support
Similarly, in your space you might be documenting new scenarios. New scenarios might not necessarily be even a pain point, but just something that's important to the business. For example, we might document how people adopt a product through a particular scenario because we know they care about it.

## Prioritize customer scenarios
## Figure out how content works across services
- https://review.docs.microsoft.com/help/contribute/contextual-toc (Contextual TOC)
- https://review.docs.microsoft.com/help/contribute/links-how-to (Links to other articles)

## Reduce feature complexity

## Perform QA of revised feature

## Define user stories  <!-- Strike, as moved to ####Identify user scenarios -->
## Write intent statemetns


## Determine content structure
- https://review.docs.microsoft.com/help/contribute/contribute-get-started-mvc (MVC)
- https://review.docs.microsoft.com/help/contribute/content-type-comparison (content type)
...
## Identify SEO terms
- https://review.docs.microsoft.com/help/contribute/contribute-how-to-write-seo-basics

## Identify term you want to use elsewhere throughout the doc
- https://review.docs.microsoft.com/help/contribute/product-names-terms (Terminology)
- https://review.docs.microsoft.com/help/contribute/azure-devops-terminology-guidance (Azure terminology)
- https://styleguides.azurewebsites.net/StyleGuide/Read?id=2696 (Cloud style)
- https://styleguides.azurewebsites.net/StyleGuide/Read?id=2700 (Microsoft style)
- https://review.docs.microsoft.com/help/contribute/file-path-guidelines (Filename)
- https://review.docs.microsoft.com/help/contribute/metadata-overview (Metadata overview)
- https://review.docs.microsoft.com/help/contribute/metadata-changes (Request metadata)
- https://review.docs.microsoft.com/help/contribute/new-azure-service-checklist (Azure checklist)
- https://review.docs.microsoft.com/help/contribute/text-formatting-guidelines
- https://review.docs.microsoft.com/help/contribute/technical-principles-checklist (Code Conventions)
...
|Subtask|Begins with|Ends by
| :-- | :-- | :--
|Create a release branch|The Product and Content team has decided that content is kept internal and is not published until a specified date. A release branch is a git branch that is created to hold documentation that needs to be hidden from the public. Therefore, a release branch is isolated from the master branch.|A release branch is created in the upstream private repo and its name is shared with all authors collaborating on content release. Typically the release branch is named with the scheme release-<conference>-<technology> or simply release-<technology> 
|Create a public repo (for code)||- Go visit your new repo on GitHub to confirm it was created correctly, or <br/>- Record your work in the original work item and set the state of the work item to closed.
|Write code|Before you create an article, think about how to lead a user through creating a proof of concept.|- Develop piece of code that you trust<br/>- Tested out the scenario
|Write first draft of content|The authoring environment is set, remotes are set, local branch is created|When markdown files for your articles are created
|Create TOC|New conceptual articles in md or yaml format are created. They can be quickstarts, tutorials, samples, overview etc.|All the new conceptual articles created are linked from TOC
|Make content discoverable|Figure out how customers are going to find this documentation/content for appropriate searches.|New conceptual articles written in md. yml have all the required metadata, SEO basics applied 
|Identify rich media|- When you need to clarify instructions and provide helpful context in an article<br/>- When you identify the requirement to add a media file in the form or screenshot, conceptual art, or animated GIF to your article.<br/>- When you have steps that aren't obvious|- Need for conceptual artwork, videos are identified<br/>- Request for rich media is completed
|Create rich media|- After you identify the requirement to add a media file in the form or screenshot, conceptual art, or animated GIF to your article.|- Media assets are created<br/>- Media files are saved on ‘media’ folder<br/>- Media files are referenced with correct markdown syntax from articles
|Check Acrolinx score on VS Code|-Content is written in Markdown text, and article saved with the .md extension<br/>- Acrolinx score along with issues is displayed for each article on VS Code |- Acrolinx score for each article the contributor creates is > 85<br/>- Local preview of article is shown on VS Code window 
|Submit pull request to stage content|When your content is added to your local branch and you are now ready to push it to upstream repo|- When your pull request is successfully built without any warnings, suggestions or errors<br/>- Acrolinx score report is shown on PR comments<br/>- If PR review is enabled for the repo, Preview links are displayed on PR comments
|Validate the technical accuracy of *Docs*|- When your first draft is completed, AND<br/>- When any article has code, or steps for a procedure|- Successful completion of end to end procedure without any errors following the documentation.<br/>- Code snippets added on articles run without errors.
|Address *Docs* validation errors|When PR is opened against upstream branch and build has failed with errors|- Pull request is successfully built without any warnings, suggestions or errors<br/>- Acrolinx score report link is shown on PR comment<br/>- If PR review is enabled for the repo, Preview links are displayed on PR comment
...
# Reviewing and publishing your content
![Article creation process - Review](./media/content-dev/4-review.svg)

|Subtask|Begins with|Ends by
| :-- | :-- | :--
|Distribute doc for review|You've written, or updated, an article and created the GitHub pull request. You can now invite others to review your work and provide feedback.|You have added your reviewers to the pull request.
|Address reviewer feedback|Feedback has been received from reviewers.|Review related changes are committed to the PR. All feedback addressed.
|Debug publishing platform|One of a range of symptoms reported through GitHub UI after a PR created or updated. Could be a doc build or validation error that doesn’t block build (example: bad link, missing metadata) to a complete build failure. Sometimes  a build takes an unusually long time or fails without reporting anything through the GitHub UI.|- Docs build runs and stages content without reporting errors<br/>- There are no errors or warnings from the build process, and all merge conflicts have been resolved. The PR is mergeable.
|Address Acrolinx feedback (from the build report)|The contributor clicks the Acrolinx Scorecard link in the pull request.|The .md file is updated to address the score. This can be done multiple times.
|Address PR reviewer feedback|- PR reviewer sends feedback in response to 1) PR sign-off, or<br/>- 2) contributor request.|Contributor signs off on PR, or<br/>- PR is merged.
|Provide feedback|Based on the existing diagram flow, this is not sequential and stands outside the other tasks in this category because the rest of the task assume the contributor versus reviewer role. This task begins when the “reviewer” is notified that a review has been requested of them.|This task ends when the reviewer submits their feedback to the contributor (in the pull request?).
|Sign-off/merge pull request|Type #sign-off in the comments field of the PR (or repo-specific equivalent)|PR is merged by the PR reviewer.
|Onboard sample code repository (into the samples browser)||


## Distribute doc for review
- https://review.docs.microsoft.com/help/contribute/contribute-how-to-write-best-practices (invite others to review content)
- https://review.docs.microsoft.com/help/contribute/technical-principles-checklist (maybe)
- https://review.docs.microsoft.com/help/contribute/writing-principles
- https://review.docs.microsoft.com/help/contribute/writing-principles-checklist

## Address reviewer feedback
- https://review.docs.microsoft.com/help/contribute/writing-principles-checklist
- https://review.docs.microsoft.com/help/contribute/technical-principles-checklist

## Address Acrolinx feedback
- https://review.docs.microsoft.com/help/contribute/contribute-get-started-setup-acrolinx-overview
- https://review.docs.microsoft.com/help/contribute/contribute-get-started-setup-acrolinx-vscode
- https://review.docs.microsoft.com/help/contribute/acrolinx-min-score

## Debug publishing platform


## Address PR reviewer feedback
- https://review.docs.microsoft.com/help/contribute/contribute-how-to-write-pull-request-quality-criteria
- https://review.docs.microsoft.com/help/contribute/technical-principles-checklist
- https://review.docs.microsoft.com/help/contribute/writing-principles-checklist


### Provide feeedback
- https://review.docs.microsoft.com/help/contribute/contribute-how-to-write-best-practices (review content and provide feedback)
- https://review.docs.microsoft.com/help/contribute/contribute-how-to-write-pull-request-reviews 
- https://review.docs.microsoft.com/help/contribute/writing-principles-checklist 
- https://review.docs.microsoft.com/help/contribute/technical-principles-checklist
- https://review.docs.microsoft.com/help/contribute/contribute-how-to-write-pull-request-etiquette

Working with Service Team content
